...
def driver_schedule_drive(driver, area_id, street_id, date_str, time_str):
    ...
    new_drive = Drive(driverId=driver.id,
                      areaId=area_id,
                      streetId=street_id,
                      date=date,
                      time=time,
...
def driver_cancel_drive(driver, drive_id):
    ...
    if drive:
        ...
        street = None
        if driver.streetId is not None:
...
def driver_view_drives(driver):
    drives = Drive.query.filter_by(driverId=driver.id).all()
...
def driver_view_requested_stops(driver, drive_id):
    drive = Drive.query.get(drive_id)
    if drive:
        return drive.stops if drive.stops else []
    return []
